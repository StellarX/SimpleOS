...
void HariMain(void)
{
	...
	io_out8(PIC0_IMR, 0xf8); /* 解除中断屏蔽，允许使用PIC1和键盘以及PIT(11111000)*/
	...
	*((int *) (task_cons->tss.esp + 8)) = memtotal;
	...
	sheet_slide(sht_back, 0, 0);//虽然还未显示，但把它移到该位置，准备显示
			...
			if (256 <= i && i <= 511) { /* 键盘数据*/
				...
				if (i == 256 + 0x0f) { /* Tab键 切换字符输入位置*/
                    if (key_to == 0) {
						...
						fifo32_put(&task_cons->fifo, 2); /*向任务b发送    2表示命令行窗口光标ON */
				    } else {
                        key_to = 0;
...
//这段程序要完成的是：先涂上背景色，再在上面写字符，最后完成刷新
void putfonts8_asc_sht(struct SHEET *sht, int x, int y, int c, int b, char *s, int l)
{
...
void console_task(struct SHEET *sheet, unsigned int memtotal)
{
	...
	int i,x,y, fifobuf[128], cursor_x = 16, cursor_c = -1, cursor_y = 28;
                ...
                if (i == 8 + 256) {
                    ...
                    if (strcmp(cmdline, "mem") == 0) {
                        ...
                        for (x = 0; x < 224; x++) {
                            ...
                            if (finfo[x].name[0] != 0xe5) {//代表这个文件没有被删除
                    ...
                    if (cursor_x < 240) {
                        ...
                        s[0] = i - 256;
                        s[1] = 0;
						cmdline[cursor_x / 8 - 2] = i - 256;//将字符存入cmdline
