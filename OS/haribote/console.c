		...
		if (fifo32_status(&task->fifo) == 0) {
			...
			{ /*键盘数据（通过任务A） */
                if (i == 8 + 256) {
                    ...
                    /*回车键*/
					/*将光标用空格擦除后换行*/
					cons_putchar(&cons, ' ', 0);//当move为0时，不使光标前进
                    ...
                    if (cons.cur_x < 240) {
                        /*显示一个字符之后将光标后移一位  */
						cmdline[cons.cur_x / 8 - 2] = i - 256;//将字符存入cmdline
...
void cons_runcmd(char *cmdline, struct CONSOLE *cons, int *fat, int memtotal)
{
	...
	} else if (strcmp(cmdline, "exit") == 0 && cons->sht != 0) {
        cmd_exit(cons, fat);
...
void cmd_dir(struct CONSOLE *cons)
{
	...
	char s[30];
	for (i = 0; i < 224; i++) {
		if (finfo[i].name[0] == 0x00) {//代表这一段不包含任何文件名信息
			break;
		}
		if (finfo[i].name[0] != 0xe5) {//代表这个文件没有被删除
...
void cmd_type(struct CONSOLE *cons, int *fat, char *cmdline)
{
	...
	if (finfo != 0) {
		...
		file_loadfile(finfo->clustno, finfo->size, p, fat, (char *) (ADR_DISKIMG + 0x003e00));//将文件内容读入内存p
		cons_putstr1(cons, p, finfo->size);
		memman_free_4k(memman, (int) p, finfo->size);//释放用于临时存放文件内容的内存空间
	} else {
		/* 没有找到文件的情况 */
...
int cmd_app(struct CONSOLE *cons, int *fat, char *cmdline)
{
	...
	if (finfo != 0) {
		...
		if (finfo->size >= 36 && strncmp(p + 4, "Hari", 4) == 0 && *p == 0x00) {
            ...
            set_segmdesc(task->ldt + 0, finfo->size - 1, (int) p, AR_CODE32_ER + 0x60);//将程序文件所在的段注册到GDT的1003号
            set_segmdesc(task->ldt + 1, segsiz - 1,      (int) q, AR_DATA32_RW + 0x60);//注册应用程序专用的内存空间
            for (i = 0; i < datsiz; i++) {
                q[esp + i] = p[dathrb + i];//将.hrb文件中的数据部分先复制到数据段后再启动程序。
            }
            start_app(0x1b, 0 * 8 + 4, esp, 1 * 8 + 4, &(task->tss.esp0)); //准备启动应用程序
